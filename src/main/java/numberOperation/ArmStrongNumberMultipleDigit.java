...
public class ArmStrongNumberMultipleDigit {
	...
	public void checkArmStrongNumber(int number) {
		...
		// Finding the digit in the number to use the power further.
		while (temp0 > 0) {
			temp0 = temp0 / 10;
			power++;
		}
	...
	public static void main(String[] args) {
		...
		ArmStrongNumberMultipleDigit obj = new ArmStrongNumberMultipleDigit();
		System.out.println("Please enter the number to check Armstrong");
		Scanner sc = new Scanner(System.in);
		int number = sc.nextInt();

		if (number < 0) {
			System.out.println("Please enter an valid positive Integer number");
